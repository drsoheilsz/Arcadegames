 ...
 *
 * @param string $new_value License status.
 * @return string
 */
 ...
 *
 * @param  string $schluessel Schluessel.
 * @return boolean
 */
function myarcade_get_license_data( $schluessel ) {
	...
	$response = wp_remote_post(
		MYARCADE_UPDATE_API . 'verify.php',
		array(
			'timeout'  => 45,
			'blocking' => true,
			'headers'  => array(
				'user-agent' => 'MyArcadePlugin/' . $wp_version . '; ' . get_bloginfo( 'url' ),
			),
			'body'     => $params,
		)
	);
