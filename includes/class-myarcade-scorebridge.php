...
class MyArcade_ScoreBridge {
  ...
  /** @var array Submitted data by the game $_POST */
  var $data = array();
  ...
  private function get_data() {
    ...
    $this->data['game_id'] = filter_input( INPUT_POST, 'game_id', FILTER_SANITIZE_NUMBER_INT );
    $this->data['user_id'] = filter_input( INPUT_POST, 'user_id', FILTER_SANITIZE_NUMBER_INT );
      ...
      case 'score': {
        ...
        if ( $score ) {
          $score_array = array(
            ...
            'score'     => $score,
            'sortorder' => get_post_meta( $this->data['game_id'], 'mabp_score_order', true ),
          );
